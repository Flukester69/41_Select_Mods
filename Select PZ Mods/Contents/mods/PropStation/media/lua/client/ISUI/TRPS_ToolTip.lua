...
TRPS_ToolTip = ISPanel:derive("TRPS_ToolTip");
...
function TRPS_ToolTip:initialise()
	ISPanel.initialise(self);
end


function TRPS_ToolTip:setItem(it)
	self.Item = it
end

function TRPS_ToolTip:isItem()
	return self.Item 
end


function TRPS_ToolTip:setName(name)
	self.name = name;
end

function TRPS_ToolTip:setContextMenu(contextMenu)
	self.contextMenu = contextMenu;
end

function TRPS_ToolTip:setTexture(textureName)
	self.texture = getTexture(textureName);
end

function TRPS_ToolTip:onMouseDown(x, y)
	return false
end

function TRPS_ToolTip:onMouseUp(x, y)
	return false
end

function TRPS_ToolTip:onRightMouseDown(x, y)
	return false
end

function TRPS_ToolTip:onRightMouseUp(x, y)
	return false
end

function TRPS_ToolTip:prerender()
...
function TRPS_ToolTip:doLayout()
	local textX = 10
	local textY = 0
	local textWidth = 0
	local textHeight = 0
...
function TRPS_ToolTip:setDesiredPosition(x, y)
	self.desiredX = x
	self.desiredY = y
...
function TRPS_ToolTip:adjustPositionToAvoidOverlap(avoidRect)
...
function TRPS_ToolTip:placeAbove(r1, r2)
	local r = r1
	r.y = r2.y - r.height - 8
	r.y = math.max(0, r.y)
	return r
...
function TRPS_ToolTip:setOwner(ui)
	self.owner = ui
end
