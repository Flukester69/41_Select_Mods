...
--[[	if getCore():getGameMode()=="LastStand" then
    ...
    if body then
        if playerInv:contains("HuntingKnife") or playerInv:contains("KitchenKnife") or playerInv:contains("Axe") then
			local knife = nil;
			local axe = nil;
			...
			if playerInv:getBestCondition("KitchenKnife") then	--playerInv:contains("HuntingKnife")
...
ISDissectCorpseContextMenu.onDissectCorpse = function(worldobjects, player, corpse, knife)
    ...
    if corpse:getSquare() and luautils.walkAdj(playerObj, corpse:getSquare()) then
		...
		if playerObj:getPrimaryHandItem() then
			if playerObj:getPrimaryHandItem():getType() == "HuntingKnife" or playerObj:getPrimaryHandItem():getType() == "KitchenKnife" or playerObj:getPrimaryHandItem():getType() == "Axe" then
